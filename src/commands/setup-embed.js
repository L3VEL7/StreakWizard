                ...
                try {
                    ...
                    if (i.customId === 'raid_cooldown_toggle') {
                        try {
                            ...
                            await streakManager.updateRaidConfig(i.guildId, {
                                ...raidConfig,
                                cooldownEnabled: newCooldownEnabled
                            });
                            ...
                            // Get fresh raid config to ensure we have the latest data
                            const updatedRaidConfig = await streakManager.getRaidConfig(i.guildId);
                            console.log('Updated cooldown enabled status:', updatedRaidConfig.cooldownEnabled);
                            
                            // Create embed with the updated configuration
                            const embed = new EmbedBuilder()
                                .setColor('#0099ff')
                            ...
                            // Create toggle cooldown button with correct state
...
async function handleRaidConfig(interaction, originalInteraction) {
    const raidConfig = await streakManager.getRaidConfig(interaction.guildId);
    console.log('Raid Config in handleRaidConfig:', raidConfig);
    ...
    console.log('Raid Config for display:', config);
...
async function handleRaidSuccessChance(interaction, originalInteraction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`Configure the base chance of success for raids.\n\n**Advanced Difficulty System:**\n• All raiders get +5% base bonus when initiating a raid\n• Difficulty adjustments based on streak ratio:\n  - Significant underdog (< 0.5 ratio): -15% success chance\n  - Moderate underdog (< 0.75 ratio): -10% success chance\n  - Slight underdog (< 1 ratio): -5% success chance\n  - Slight overdog (> 1 ratio): +5% success chance\n  - Moderate overdog (> 1.5 ratio): +10% success chance\n  - Significant overdog (> 2 ratio): +15% success chance\n\n**Target Value Bonus:**\n• Progressive bonus based on target's streak size:\n  - Streaks 10-24: +3% success chance\n  - Streaks 25-49: +6% success chance\n  - Streaks 50-74: +9% success chance\n  - Streaks 75-99: +12% success chance\n  - Streaks 100+: +15% success chance\n\n**Underdog Reward System:**\n• When a player raids someone with significantly more streaks:\n  - Significant underdog: +25% max steal, 60% risk reduction\n  - Moderate underdog: +15% max steal, 40% risk reduction\n  - Slight underdog: +10% max steal, 20% risk reduction\n\nThis creates dynamic gameplay where success chances range from ~35% for very hard raids to ~75% for very easy ones, making strategic choices meaningful!`)
        .addFields(
            { name: 'Current Setting', value: `${currentChance}% base chance + difficulty adjustments and bonuses` }
        );
...
async function handleTriggerWords(interaction, originalInteraction) {
    try {
        ...
        const triggerWordsEmbed = new EmbedBuilder()
            .setColor('#0099ff')
            .setTitle('🔤 Trigger Word Management')
            .setDescription('Configure words that trigger streak counting.')
            .addFields(
                { name: 'Current Trigger Words', value: triggerWords.length > 0 ? triggerWords.join(', ') : 'No trigger words set.' },
        ...
        // Create buttons for trigger word management
        const row = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
        ...
        await interaction.update({
            embeds: [triggerWordsEmbed],
            components: [row]
        });

        // Set up collector for button interactions
        const filter = i => i.user.id === originalInteraction.user.id;
        const collector = interaction.channel.createMessageComponentCollector({ filter, time: 300000 });

        collector.on('collect', async i => {
            if (i.customId === 'back_to_main') {
                collector.stop();
                await showMainOptions(originalInteraction);
                return;
            }
            ...
            if (i.customId === 'add_trigger_word') {
                ...
                const wordInput = new TextInputBuilder()
                    ...
                    .setLabel('Enter a word that will trigger streak counting')
            ...
            if (i.customId === 'remove_trigger_word') {
                // Only show remove options if there are trigger words
                if (triggerWords.length === 0) {
                    await i.reply({
                        content: 'There are no trigger words to remove.',
                        ephemeral: true
                    });
                    return;
                }

                // Create select menu with trigger words
                const selectRow = new ActionRowBuilder()
                    .addComponents(
                        new StringSelectMenuBuilder()
                            .setCustomId('remove_trigger_word_select')
                            .setPlaceholder('Select words to remove')
                            .setMinValues(1)
                            .setMaxValues(Math.min(triggerWords.length, 25))
                            .addOptions(triggerWords.map(word => ({
                                label: word,
                                value: word
                            })))
                    );
                ...
                await i.reply({
                    content: 'Select the trigger words you want to remove:',
        ...
        const modalListener = async interaction => {
            ...
            try {
                const word = interaction.fields.getTextInputValue('trigger_word_input').toLowerCase().trim();
                
                // Validate the word
                if (word.length < 2) {
                    await interaction.reply({
                        content: 'Trigger words must be at least 2 characters long.',
                        ephemeral: true
                    });
                    return;
                }
        ...
        const selectListener = async interaction => {
            ...
            try {
                ...
                await interaction.reply({
                    content: 'An error occurred while removing trigger words.',
                    ephemeral: true
                });
            }
        };
        ...
        await interaction.update({
            content: '❌ An error occurred while loading trigger word settings.',
            components: []
        });
    }
}
...
async function showMainMenu(interaction, guildId) {
    try {
        ...
        console.log('Main menu raid config:', raidConfig);
